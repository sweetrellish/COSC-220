...
int main() {
  ...
  while (getline(dataFile, line)) {
    ...
    int lastpos = pos;
    pos = line.find('\t', pos + 1);
    airline.SeatKMWK = stol(trim_copy(line.substr(lastpos, pos - lastpos)));

    lastpos = pos;
    pos = line.find('\t', pos + 1);
    airline.Incidents_85_99 =
        stol(trim_copy(line.substr(lastpos, pos - lastpos)));
    ...
    //		airline.Fatalities_00_14 = stol(
    //				trim_copy(line.substr(lastpos, pos - lastpos)));

    airline.Fatalities_00_14 = stol(trim_copy(line.substr(lastpos)));
